...
```sh
smdr README.md                        # open a file
smdr -w README.md                     # watch mode — auto-reloads on save
smdr README.md CHANGELOG.md           # open multiple files as tabs
cat README.md | smdr                  # read from stdin
smdr -t dracula README.md             # open with a specific theme
```

smdr keeps everything in one window. Running `smdr <file>` while a window is
already open adds the file as a new tab in the **existing** window rather than
launching a new one. Re-opening a file that is already open switches to its tab
...
smdr ships with 22 built-in themes. Set one with `-t` / `--theme`, cycle
through them at runtime with `Ctrl-T`, or list all names with `--list-themes`.
